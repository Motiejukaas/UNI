...
//example of changing the value of x_above from the address using a function
void findNearestIntAboveAvg(int array[], double avg, int N, int *x_above) {
    *x_above = array[0];
    for (int i = 0; i < N; ++i) {
        if (array[i] > *x_above) {
            *x_above = array[i];
...
int main() {
    ...
    if (N == 0) {
        ...
        findNearestIntAboveAvg(array, avg, N, &above_avg);
